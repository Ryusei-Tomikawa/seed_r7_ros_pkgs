...
import time
import rospy
from rsdlab_fcsc_pkg.nodes.seednoid_manipulation import Flag
##-- for smach
...
class NaviAction:
  def __init__(self):
    ...
    rospack.list() 
    path = rospack.get_path('rsdlab_fcsc_pkg')
...
class PERSON_DETECT(State):
    def __init__(self, _number, _flag):
        State.__init__(self, outcomes=['succeeded','aborted'])
        self._number = _number
        _flag = Flag.data
        self._flag = _flag
    
    def execute(self, _number, _flag):
        if self._number == 1:
            if self._flag:
                return 'succeeded'
            else:
...
# class OUTPUT_SOUND(State):
#     def __init__(self, _flag):
#         State.__init__(self, outcomes=['succeeded','aborted'])
#         _flag = Flag.data
#         self._flag = _flag

#     def execute(self, _flag):
#         if 
#==================================
#==================================
if __name__ == '__main__':
    rospy.init_node('fcsc_node')
    ...
    ####################### Person detect #####################################
    person_detect = StateMachine(outcomes=['succeeded', 'aborted']) 
    with person_detect:
        StateMachine.add('PERSON DETECT 1', PERSON_DETECT(1),\
            transitions={'succeeded': 'succeeded', 'aborted':'aborted'}) # down lifter
        # StateMachine.add('PERSON DETECT 2', PERSON_DETECT(2),\
        #     transitions={'succeeded': 'succeeded', 'aborted':'aborted'}) # manipu start
    ############################################################################

    ######################## OUTPUT SOUND ######################################
    # output_sound = StateMachine(outcomes=['succeeded', 'abroted'])
    # with output_sound:
    #     StateMachine.add('OUTPUT SOUND', OUTPUT_SOUND(),\
    #     transitions={'succeeded': 'MOVE_STOP', 'aborted': 'abroted'})
    ############################################################################
    ...
    with scenario_play:
        StateMachine.add('PERSON DETECT 1', person_detect,\
