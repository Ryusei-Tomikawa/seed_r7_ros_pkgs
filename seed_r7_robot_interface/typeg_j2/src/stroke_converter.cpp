...
void seed_converter::TypeG_J2::makeTables()
{
...
void seed_converter::TypeG_J2::Angle2Stroke
(std::vector<int16_t>& _strokes, const std::vector<double>& _angles)
{
  ...
  _strokes[20]
    = static_cast<int16_t>(scale * setAngleToStroke(90 + rad2Deg * _angles[20], elbow_p.table));
  ...
  _strokes[28]
    = static_cast<int16_t>(-scale * rad2Deg * _angles[28]);  //knee
  _strokes[29]
    = static_cast<int16_t>(scale * setAngleToStroke(  rad2Deg * _angles[29], leg.table));  //ankle
}

void seed_converter::TypeG_J2::Stroke2Angle
(std::vector<double>& _angles, const std::vector<int16_t>& _strokes)
{
  ...
  _angles[20] = - (M_PI/2) + deg2Rad * setStrokeToAngle(scale_inv * _strokes[20], elbow_p.inv_table);
  ...
  _angles[28] = - deg2Rad * scale_inv * _strokes[28];// knee
  _angles[29] = deg2Rad * setStrokeToAngle(scale_inv * _strokes[29], leg.inv_table);  // ankle
}

PLUGINLIB_EXPORT_CLASS(seed_converter::TypeG_J2, seed_converter::StrokeConverter)
